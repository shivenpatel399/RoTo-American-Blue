...
void mock_mod_angle(float degrees) {
  if (degrees > -10 && degrees < 10) {
    ...
    mod_gyro_angle = 88.5 + halfTarget;
  }
  else if (degrees > 167 && degrees < 187) {
    mod_gyro_angle = 177 + halfTarget;
  }
  else if (degrees > 255.5 && degrees < 275.5)  {
    mod_gyro_angle = 265.5 + halfTarget;
  }
  else if (degrees > 344 && degrees < 364) {
    mod_gyro_angle = 354 + halfTarget;
  }
  else if (degrees < -78.5 && degrees > -98.5) {
    mod_gyro_angle = -88.5 + halfTarget;
  }
...
void speedTime_control(float Time) { // Critical Change Point
  ...
  remaining_seconds = Time - 2 - (Turns * 1) - (cycle * 0.35) - 0.35;
