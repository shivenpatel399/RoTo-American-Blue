...
void north() { // Critical Change Point and other parts
  if (moving == false) {
    ...
    PDnorth(counts, powerneeded, gainNS);
  }
}
...
void south() { // Critical Change Point and other parts
  if (moving == false) {
    ...
    PDsouth(counts, powerneeded, gainNS);
  }
}
...
void east() { // Critical Change Point and other parts
  if (moving == false) {
    ...
    PDeast(counts, powerneeded, gainEW);
  }
}
...
void west() { // Critical Change Point and other parts
  if (moving == false) {
    ...
    PDwest(counts, powerneeded, gainEW);
  }
}
...
void speedTime_control(float Time) { // Critical Change Point
  ...
  remaining_seconds = Time - 2.1 - (cycle * 0.5); // Add time for Half step
  total_distance = (Straights * 50); // no half step cuz halfstep is constant
  distovertime = total_distance/remaining_seconds;
  powerneeded = (4.18*distovertime) + 28.6;
 // please edit this
  counts = 586 + (0.0982 * powerneeded) + ((-3.78 * pow(10, -3)) * pow(powerneeded, 2)) + ((9.97 * pow(10, -6)) * pow(powerneeded, 3));
  gainNS = 4.11 + (0.0701 * powerneeded) + ((-2.94 * pow(10, -4)) * pow(powerneeded, 2)) + ((6.67 * pow(10, -7)) * pow(powerneeded, 3));
  gainEW = 0.746 + (0.136 * powerneeded) + ((-6.93 * pow(10, -4)) * pow(powerneeded, 2)) + ((1.47 * pow(10, -6)) * pow(powerneeded, 3));
}
...
void executeRobot() {
  ...
  for (int j = 0; j < cycle; j++) {
    delay(250); // Increase Delay for accurate angle stop, tradeoff is speed. Make it around 400-500
...
void loop() {
  /*
   Basic code to run repeatedlyza
  */
}
...
To DO
Need to do: Half PD, Back PD, and Time Adjustment
*/