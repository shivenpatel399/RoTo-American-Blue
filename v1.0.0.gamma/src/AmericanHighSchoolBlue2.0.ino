...
void north() { // Critical Change Point and other parts
  if (moving == false) {
    ...
    PDnorth(CMtoSteps(50), powerneeded, gainNS);
  }
}
...
void south() { // Critical Change Point and other parts
  if (moving == false) {
    ...
    PDsouth(CMtoSteps(50), powerneeded, gainNS);
  }
}
...
void east() { // Critical Change Point and other parts
  if (moving == false) {
    ...
    PDeast(CMtoSteps(50), powerneeded, gainEW);
  }
}
...
void west() { // Critical Change Point and other parts
  if (moving == false) {
    ...
    PDwest(CMtoSteps(50), powerneeded, gainEW);
  }
}
...
void speedTime_control(float Time) { // Critical Change Point
  ...
  // Speed over Time control
  remaining_seconds = Time - 4.01 - (cycle * 0.35); // Add time for Half step
  total_distance = Straights * 50; // no half step cuz halfstep is constant
  distovertime = total_distance/remaining_seconds;
  powerneeded = -25.1 + (0.669 * distovertime) + ((-1.04 * pow(10, -3)) * pow(distovertime, 2)) + ((1.84 * pow(10, -6)) * pow(distovertime, 3)); // please edit this
  counts = (0.0115 * powerneeded) + 549;
  gainNS = 4.84 + ((-8.06 * pow(10, -3)) * powerneeded) + ((8.21 * pow(10, -5)) * pow(powerneeded, 2));
  gainEW = 0.746 + (0.136 * powerneeded) + ((-6.93 * pow(10, -4)) * pow(powerneeded, 2)) + ((1.47 * pow(10, -6)) * pow(powerneeded, 3));// yea this too
}
